...
sql_str = """ SELECT *  INTO movies_top FROM  
        (
	select movieid, first_value(ratings_num) over (partition by movieid) as ratings_num, first_value(ratings_avg) over (partition by movieid) as ratings_avg
	FROM
		(
		select movieid, count(rating) over (partition by movieid) as ratings_num, avg(rating) over (partition by movieid) as ratings_avg from ratings
		) a
        group by movieid, ratings_num, ratings_avg
	) b
        """
...
grouped_tags = tags_df.groupby('tags').count()
sorted_tags = grouped_tags.sort_values(by=['movieid'], ascending=False)
